...
####################
sub tag_extract {					# Get portion of html code defined by tag like <div></div> etc.
####################
my ($code, $pattn, $tag_type, $tag_trim) = @_;

#	code - just text
#	pattn to search start of tag
#	word to search end of tag
#	when wantarray tag_trim prevents <tag> dubling (cuts from before/after)

my $div_start = 0;
my $div_end = length( $code );
my $div_count = 1;
my $found_at = 0;
my @start = ();		# Where tag starts
my @end = ();		# Where tag ends
my $tag_len = 0;	# To calculate end of section

my $pattn_nest = '<\s*'.$tag_type.'[^>]*>';
	$pattn_nest= qr/$pattn_nest/;			# To find any nested similar tags
	$tag_type =~ s/\W//;			# Remove any brackets
my $pattn_end = '<\s*\/\s*'.$tag_type.'[^>]*>';

	$pattn_end = qr/$pattn_end/;			# To find closing tags
	$pattn = qr/$pattn/;				# Tag to find

	while ( $code =~ m/(<[^>]+>)?/g ) {		# Found something like tag
		my $found_at = pos( $code );		# Found where?
		my $tag = $1;
		$tag_len = length($tag);

		if ( $tag =~ /$pattn_end/i ) {		# Tag closed, store closing position,
			$div_end = $found_at - $tag_len * ( $tag_trim ? 0 : 1 );	# - ( $tag_trim ) store closing position (with(out) tag len)
			$div_count--;
		} elsif ( $tag =~ /$pattn/i ) {
			$div_count = 1;			# Reset counter to first occurience
			$div_start = $found_at - ( $tag_len * $tag_trim );	# Store position (with(out) tag len)
		} elsif ( $tag =~ /$pattn_nest/i ) {	# Found nested tag
			$div_count++;
		}
		last if ($div_count == 0) && $div_start;
	}

	if ( wantarray() ) {
		return ( substr( $code, 0, $div_start ),
			substr( $code, $div_end, length($code)-$div_end),
			substr( $code, $div_start, $div_end-$div_start));
			# Code before and After tag container and the container
	} else {
		return substr( $code, $div_start, $div_end-$div_start+$tag_len);
			# Portion of tag container
	}
}
####################
sub js_string {					# Make 'string\n'+'string'
####################
my @text = split( /[\n\r]+/, shift );
my $ret = '';
my $cnt = 0;
...
#########################
sub fget {				# Extract correct filename from upoad field
#########################
my $xfilename = shift;
my $ext = '';
	$xfilename = main::validname($xfilename);		# Zap gremlins
	if ( $xfilename =~ /\b([\w\-\.\$!]+)$/ ) {
		$xfilename = $1;						#  Extract original filename from full path
		my @filename = split(/\./, $xfilename);
		$xfilename = $filename[0];
		$ext = $filename[$#filename];
	} else {
		$xfilename = time;		# Random filename w/o extension
		$ext = 'txt';			# Dummy extension
	}					#  getting filename from fullpath
	if ( wantarray() ) {
		return ( $xfilename, $ext);
	} else {
		return "$xfilename.$ext";
	}
}
